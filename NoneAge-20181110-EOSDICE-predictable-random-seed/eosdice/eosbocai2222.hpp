...
class eosbocai2222 : public contract
{
  ...
  private:
    ...
    uint8_t random(account_name name, uint64_t game_id)
    {
        ...
        print("[DICE RANDOM]tapos_block_prefix=>",(uint64_t)tapos_block_prefix(),"|tapos_block_num=>",(uint64_t)tapos_block_num(),"|name=>",name,"|game_id=>",game_id,"|current_time=>",current_time(),"|total=>",total_eos.amount,"\n");
